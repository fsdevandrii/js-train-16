...
function filterStudentsByGrade(students, grade) {
  ...
  // Очистимо консоль перед виведенням повідомлення
  console.clear();

  // Виведемо повідомлення для відстеження роботи програми
  console.log(
    `Відфільтруємо тільки тих студентів оцінка яких співпадає з ${grade}`
  );

  // Пройдемо по кожному елементу масиву `students`
  students.forEach((student) => {
    // Перевіримо, чи співпадає оцінка студента зі значенням змінної `grade`
    if (student.grade === grade) {
      // Якщо оцінка співпадає, виведемо повідомлення з ім'ям студента
      console.log(`Студент з ім'ям ${student.name} має оцінку ${grade}`);
    }
  });
}
...
function logArrayElements(arr) {
  ...
  if (!Array.isArray(arr)) {
    // Якщо arr не є масивом, виводимо в консоль критичну помилку з текстом "Аргумент має бути масивом!"
...
function warnAboutMaxArrayLength(arr, maxLength) {
  ...
  if (!Array.isArray(arr) || typeof maxLength !== "number") {
    // Якщо arr не є масивом або maxLength не є числом, виводимо в консоль критичну помилоку з текстом: "Перший аргумент має бути масивом, другий аргумент має бути числом!".
    console.error(
  ...
  if (arr.length > maxLength) {
    ...
    // Якщо довжина масиву не перевищує максимальну, виводимо в консоль корисне повідомлення з текстом: "Довжина масиву не перевищує максимально допустиму.".
...
function compareArrays(arr1, arr2) {
  ...
  // Оскільки напряму порівняти масиви за допомогою оператора === не можливо, перетворимо їх в JSON і порівняємо.
  const areEqual = JSON.stringify(arr1) === JSON.stringify(arr2);

  // Використовуємо `console.assert()`, щоб вивести помилку, якщо масиви не однакові.
  console.assert(areEqual, "Масиви не однакові!");
}
...
function buildMatrix(size) {
  ...
  if (typeof size !== "number") {
    // Якщо size не є числом, виводемо в консоль критичну помилку з текстом "Аргумент має бути числом!".
  ...
  for (let i = 0; i < size; i++) {
    // Створюємо порожній масив, який буде рядком матриці.
    let row = [];
    // Цикл всередині циклу для заповнення колонок матриці, лічильник j має бути від 0 до розміру матриці.
  ...
  // Виводимо матрицю в консоль за допомогою таблиці.
...
function displayGroupedInfo(students) {
  ...
  // Переберемо об'єкти за допомогою циклу for...of
...
function validateUserInput(userInput) {
  ...
  // Перевіряємо умову наявності імені користувача, якщо воно відсутнє, виводимо "Помилка: ім'я користувача відсутнє!"
  if (!userInput.username) {
    console.warn("Помилка: ім'я користувача відсутнє!");
  }

  // Перевіряємо умову наявності паролю, якщо він відсутній, виводимо "Помилка: пароль відсутній!"
  if (!userInput.password) {
    console.warn("Помилка: пароль відсутній!");
  }

  // Перевірка довжини паролю, якщо вона менше 8 символів, виводимо повідомлення про потенційну проблему з текстом "Попередження: пароль має бути довшим за 8 символів!"
  console.assert(
...
function countVowelsAndConsonants(word) {
  ...
  // Створюємо рядок, де будуть всі голосні літери "aeiou"
  const vowels = "aeiou";
  ...
  // Перебираємо кожну літеру у слові за допомогою циклу for...of
  for (let letter of word) {
    ...
    if (vowels.includes(letter)) {
      vowelCount++; // Якщо так, збільшуємо лічильник голосних
    } else if (letter >= "a" && letter <= "z") {
      consonantCount++; // Якщо літера - літера англійського алфавіту (приголосна), збільшуємо лічильник приголосних
    }
  }
